import pytest
from letter_box.utils import ValidLiterals, CircularIterator, can_make_word
...
def test_can_make_word():
    ...
    assert list(can_make_word("map", letters, S)) == [[11, 3, 8], [11, 4, 8]]
    ...
    assert list(can_make_word("paan", letters, S)) == [[8, 3, 10, 5], [8, 4, 10, 5]]
    ...
    #    P G
    # --------------------------------
    letters = {"u": {0}, "g": {1, 4}, "a": {2}, "n": {3}, "p": {5}, "i": {6}, "m": {7}}
    # These are valid words
    assert list(can_make_word("magi", letters, S)) == [[7, 2, 1, 6], [7, 2, 4, 6]]
    assert list(can_make_word("pig", letters, S)) == [[5, 6, 1], [5, 6, 4]]
