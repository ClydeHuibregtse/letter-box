...
from .graphs import Trajectory, GraphNode
    ...
    for i, l in enumerate(letters):
        ...
        for t_idx in range(N):
            ...
            if best_trajectory is None or (
                len(t.words) <= len(best_trajectory.words) and not t.is_fail()
            ):
                best_trajectory = t
            # Increment our temperature
            T -= t_idx / N

        assert best_trajectory is not None, f"No solution found for '{l}' at index {i}"
